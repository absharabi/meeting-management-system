...
const buildCoverPageContent = (meeting: IMeeting) => {
  const cover = {
    meetingTitle: meeting.title || meeting.momCoverDetails?.meetingNumber || 'Meeting',
    meetingType: meeting.meetingType || meeting.momCoverDetails?.meetingBody || 'Meeting',
  ...
  return [
    ...
    '0 0.62 0.25 rg',
    '297 602 Td',
    `(${escapePdf(cover.meetingTitle)}) Tj`,
    'ET',
    'BT',
    '/F2 14 Tf',
    '0.80 0 0 rg',
    '297 548 Td',
    `(${escapePdf(cover.meetingType)}) Tj`,
    'ET',
    'BT',
    '/F1 18 Tf',
    '0.80 0 0 rg',
